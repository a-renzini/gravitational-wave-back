        ...
        for idx_b in range(self._nbase):
            ...
            for ip in range(npix_out):
                ...
                for jp in range(ip,npix_out):
                    ...
                    if npol == 1:       #edit !!
                        val = np.einsum('i,j -> ij', 2.*(4.*np.pi)**2/npix_out**2 * delf**2 * gamma_rot_ud[ip] * np.sum(window[:]**2 * Ef[:]**2/ pf[:]*(np.cos((bdotp[ip]-bdotp[jp])*freq[:]) + 1.j * np.sin((bdotp[ip]-bdotp[jp])*freq[:]) )), gamma_rot_ud[jp])
                    
                        M_pp[ip,jp] += val
                        
                        if ip!= jp : M_pp[jp,ip] += np.conj(val)        #edit !!
