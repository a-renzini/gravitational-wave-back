...
start   = 1126072320 #start = start time of O1 ...
...
for sdx, (begin, end) in enumerate(zip(segs_begin,segs_end)):
    ...
    while idx_block < len_ctime:
        ...
        if len_ctime_nproc == nproc:   # when you hit nproc start itearation
            ...
            z_p = np.zeros(npix_out*npol,dtype=complex)
            my_M_p_pp = np.zeros((npix_out*npol,npix_out*npol),dtype=complex)
            cond = 0.
            ...
            if ISMPI:    
                comm.barrier()
                z_buffer = comm.reduce(z_p, root = 0, op = MPI.SUM)
            ...
            if myid == 0:
                ...
                if counter % (nproc*step) == 0 or checkpoint == True:    
                    ...
                    print 'saved dirty_map and checkfile @ min', counter, 'with endtime', endtime, '; avoided ', avoided, ' mins.'
