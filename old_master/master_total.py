...
for sdx, (begin, end) in enumerate(zip(segs_begin,segs_end)):
    ...
    for idx_t, ct_split in enumerate(ctime):
        #print ct_split
        stream.append(run.scan(ct_split, 0., 2000., m_lm))
    ...
    for idx_str, (h1, l1) in enumerate(zip(strain_H1, strain_L1)):
        ...
        strain_H1_w = strain_H1_f/(h1_psd)#*np.sqrt(2.)
        strain_L1_w = strain_L1_f/(l1_psd)#*np.sqrt(2.)
        ...
        while idx_a > low_f_idx-1 and idx_a < high_f_idx+1:
            #print idx_a, cf
            f_x_coar.append(np.mean(freqs[idx_a:idx_a+cf])) #probably redundant
            s_H1_coar.append(np.mean(strain_H1_f[idx_a:idx_a+cf])*np.sqrt(cf))
            s_L1_coar.append(np.mean(strain_L1_f[idx_a:idx_a+cf])*np.sqrt(cf))
            idx_a = idx_a+cf
        ...
        #print np.var(np.real(s_H1_coar)), np.var(np.imag(s_H1_coar)), np.var(np.real(strain_H1_f[low_f_idx:high_f_idx])), np.var(np.imag(strain_H1_f[low_f_idx:high_f_idx]))
        #print np.mean(s_H1_coar)
        #print np.mean(strain_H1_f)
        
        # plt.figure()
        # plt.plot(freqs[low_f_idx:high_f_idx], np.real(strain_H1_f[low_f_idx:high_f_idx]), color = 'r')
        # plt.plot(f_x_coar, np.real(s_H1_coar), color = 'b')
        # plt.ylim([-0.1e-17,0.1e-17])
        # plt.savefig('coarsedreal.png' )

#
#         plt.figure()
#         plt.plot(freqs[low_f_idx:high_f_idx], np.imag(strain_H1_w[low_f_idx:high_f_idx]), color = 'r')
#         plt.plot(f_x_coar, np.imag(s_H1_coar), color = 'b')
#         #plt.ylim([-100.,100.])
#         plt.savefig('coarsedimag.png' )
               
        
        strain_H1_coar.append(s_H1_coar)
        strain_L1_coar.append(s_L1_coar)
        freqs_x_coar.append(f_x_coar)      
        
        p_split_1.append(H1_psd(f_x_coar)) #strain_H1_coar * np.conj(strain_H1_coar)
        p_split_2.append(L1_psd(f_x_coar))
    
    plt.figure()
    plt.plot(f_x_coar, abs(strain_H1_coar), color = 'r')
    plt.plot(f_x_coar, np.sqrt(p_split_1), color = 'b')
    #plt.ylim([-100.,100.])
    ...
    d_lm = run.projector(ctime,strain_H1_coar, strain_L1_coar,p_split_1,p_split_2,freqs_x_coar,cf=cf)
    D_lm +=d_lm/len(ctime)
    
    for idx_t, ct_split in enumerate(ctime):
        ones = [1.]*len(freqs_x_coar[idx_t])
        proj_lm[idx_t] = run.summer(ctime[idx_t],ones,freqs_x_coar[idx_t]) 
    
    #dirty_map_lm = hp.alm2map(np.sum(dt_lm,axis = 0),nside,lmax=lmax)
    dirty_map = hp.alm2map(d_lm,nside,lmax=lmax)
    ...
    for idx_t in range(len(ctime)):
            scan_lm.append(run.scanner(ctime[idx_t], p_split_1[idx_t],p_split_2[idx_t],freqs_x_coar[idx_t],cf=cf))
