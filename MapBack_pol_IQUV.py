...
def map_in_gauss(nside_in, noise_lvl):
    ...
    map3 = hp.synfast(cls, nside=nside, pol=True, new=True, verbose = False)
    map4 = hp.synfast(cls, nside=nside, pol=True, new=True, verbose = False)
    return np.vstack((map1, map2, map3, map4))*beta
...
class Telescope(object):
    ...
    def __init__(self, nside_in,nside_out,fs, low_f, high_f, dects, maptyp, this_path, noise_lvl=1, alpha=3., f0=1., data_run = 'O1',npol=1): #Dect list
        ...
        np.savez(this_path + 'map_in.npz', map_in = self.map_in)        
        #print self.map_in
...
# **************** Whitening Modules *************** 
    ...
    def simbase(self,freqs,q_n,pix_b,nbase,poi = False):
        ...
        #hp.mollview(map_in[0])
        #plt.savefig('map_in_simbase.png')
        alpha = self.alpha
        f0 = self.f0
        ...
        for idx_f,f in enumerate(freqs):   
            ...
            #Apix = np.einsum('ij,j -> ij', map_in, exp_bdotp)
            Apix = np.einsum('ij,ij -> j', map_in, gamma_rot)
            #np.array(self.npol*[np.zeros_like(map_in[0], dtype = complex)])
            
            #for i in range(len(Apix)):
            #    Apix[i] = map_in[i]*exp_bdotp            

            df[idx_f] = 4.*np.pi/npix_in * delta_freq*window[idx_f] * self.E_f(f,alpha,f0) * np.einsum('j,j->', exp_bdotp, Apix )
    ...
    def notches(self):
        ...
        if self.data_run == 'O2':             
            notch_fs = np.array([30.25, 31.25,32.25,33.0,34.5,35.25,36.25,37.0,40.5,41.75,45.5,46.0,59.6,301.0,305.0,315.4,331.5,500.25])            
    ...
    def sigmas(self):
        ...
        if self.data_run == 'O2':                         
            sigma_fs = np.array([.02,.02,.02,.02,.02,.02,.02,0.1,.01,.2,.2,.2,.2,2.,.5,.2,.1,.2])            
    ...
    def summer(self, ct_split, strains, pows, freq, pix_b, q_n , norm):     
        ...
        zp  = np.zeros(npol*npix_out, dtype = complex)
        Mpp = np.zeros((npol*npix_out,npol*npix_out), dtype = complex)
        ...
        for idx_b in range(self._nbase):
            ...
            gamma_rot_ud = hp.ud_grade(gamma_rot,nside_out = self._nside_out) 
